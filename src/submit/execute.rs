...
pub struct SubmissionResult {
    ...
    /// The owned changes with bookmarks solidified (created bookmarks resolved
    /// to real names), for rebuilding the bookmark graph post-execute.
...
mod tests {
    use super::*;
    use crate::jj::BookmarkInfo;

    /// A `--create` submit names bookmarks by their pending display form
    /// (`(new bookmark for …)`) until `execute()` solidifies them into real
    /// names via `solidify_bookmark`. `SubmissionResult.changes` must carry the
    /// POST-solidify owned changes so a later hook can rebuild the bookmark
    /// graph over real names — not the pending form.
    ///
    /// This tests the solidify+collect path at the narrowest level: a full
    /// `--create` `execute()` needs a live `RootExecuteContext` (network), so
    /// we drive `solidify_bookmark` directly and confirm the collected `Vec`
    /// reflects the solidified state that `execute()` moves into the result.
    #[test]
    fn submission_result_changes_carry_solidified_bookmark_names() {
        ...
        // Before solidify, the change has no real bookmark; a graph built now
        // would name it by the pending display form.
        let pending_display = change_id_to_temp_bookmark_name(change_id);
        assert_eq!(pending_display, "(new bookmark for abcd1234)");

        // Solidify, mirroring execute.rs's `change.solidify_bookmark(name)`.
        ...
        let names: Vec<&str> = result
            .changes
            .iter()
            .flat_map(|c| c.bookmarks.iter().map(BookmarkInfo::name))
            .collect();
        assert_eq!(names, vec![solidified]);
        assert!(
            !names.iter().any(|n| n.starts_with("(new bookmark for")),
            "changes must carry solidified names, not the pending display form"
        );
