...
mod tests {
    ...
    fn validate_rejects_whitespace_only_token() {
        // An all-whitespace literal with no tokenCommand is not a usable token.
        // validate_config must reject it loud (byte-non-empty but trim-empty),
        // matching resolved_token's trim-based emptiness so the error surfaces
        // at validate time rather than slipping through to GitHubForge::new.
        ...
        assert!(
            err.contains("github.token or github.tokenCommand is required"),
            "expected required-keys error, got: {err}"
        );
    }
}
